				...
				<!-- Header -->
			      <c:choose>
			         <c:when test="${empty loginMember}">
			            <header id="header" class="alt"> <a href="index.html" class="logo"><strong>Forty</strong> <span>by HTML5 UP</span></a>
			             <nav>
			             <a href="#menu">로그인</a>
			            </nav> </header>
			         </c:when>
			         <c:when test="${loginMember.email eq 'admin'}">
						...
						<!-- Q1. 회원가입 기능 만들기 -->
							<li><h5>회원가입</h5></li>
								<form action="memberInsert" method="post">
									<li><input type="text" name="email" id="inputE" placeholder="Email을 입력하세요"></li>
									
									<li><input type="button" value="중복체크" onclick="checkE()"></li>
									<li><span id="resultE"></spen></li>
		...
		<!-- Scripts -->
			...
			function checkE(){ /* $ : JQery */
				...
				$.ajax({
					// 어디로 요청할 것인지
					url : "checkEmail",
					// 요청데이터(JSON타입 -> {key : value, {key : value}})
					data : {"inputE" : inputE},
					// 요청방식
					type : "get",
					// 요청-응답 성공
					success : function(data){
						if (data == 1){
							$('#resultE').text('사용할 수 있는 이메일');
						}else{
							$('#resultE').text('사용할 수 없는 이메일');
						}
					},
					// 요청-응답 실패
					error : function(){
